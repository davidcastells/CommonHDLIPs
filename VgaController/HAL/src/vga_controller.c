...
void vga_set(vga_controller_dev* vga, int buffer, int bytes)
{
	IOWR_32DIRECT( vga->base_addr, 0, 0x0 ); /* Reset the VGA controller */
	IOWR_32DIRECT( vga->base_addr, 4, buffer ); /* Where our frame buffer starts */
	IOWR_32DIRECT( vga->base_addr, 8, bytes ); /* amount of memory needed */   
	IOWR_32DIRECT( vga->base_addr, 0, 0x1 ); /* Set the go bit. */
...
*
******************************************************************/
void vga_clear_screen (display_frame_buffer_struct* vga_frame_buffer, char color)
{
